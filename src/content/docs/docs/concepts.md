---
title: アーキテクチャと主要概念
description: 固定ランタイム + Pack + SDK の 4 層モデル、通信経路、Event ID、Group ID、接続シナリオ。
---
...
Hapbeat は「固定ランタイム + Pack + SDK」の 3 分離を基礎に、制御面を担う Bridge 層をオプションで挟む 4 層構成である。

- **Layer 0: 共通仕様 (hapbeat-contracts)** — 全 repo が参照する仕様の起点
- **Layer 1: 固定ランタイム (hapbeat-device-firmware)** — デバイス本体の ESP32-S3
- **Layer 2: Pack（コンテンツ配布）(hapbeat-pack-tools)** — 触覚アセットを Kit 単位でパッケージ化
...
触覚コンテンツを識別する軽量な文字列 ID。SDK は Event ID を送信し、Hapbeat 側で Pack からクリップを解決して再生する。

詳細: [Pack / Kit フォーマット](/docs/pack/)
