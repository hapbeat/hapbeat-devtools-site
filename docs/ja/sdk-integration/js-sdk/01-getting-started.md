...
kind: tutorial
description: npm で入れて数行で Hapbeat を駆動する。Node と React Native は UDP 直送、Browser は helper 経由。起点（いつ鳴らすか）と触覚の編集（何を・どう鳴らすか）を分けて event id で結ぶ JS/TS SDK。
sidebar:
  order: 1
...
Android / iOS のモバイルアプリでもコードは同じで、`react-native-udp` 経由で
**helper なしに UDP を直送**します（Android 実機・RN 0.86 / Hermes で検証済み）。
...
セットアップ（`react-native-udp` / `fast-text-encoding` の導入、metro.config.js の
