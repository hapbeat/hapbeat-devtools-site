...
## デバイス側のジッタバッファ (`streamBufferMs`)

`streamSendAheadSeconds` は **ホスト側** の送信バッファですが、`HapbeatConfig.streamBufferMs`（既定 30ms、0-500）は **デバイス側** のジッタバッファ深さです。Wi-Fi のゆらぎで CLIP の音声/触覚が途切れやすい場合はこちらを上げます（トレードオフ: レイテンシ増）。

- `0`（既定挙動を明示的に指定しない値）= 送らない。デバイスは低遅延モード（アンダーラン時に再prime）のまま
- `>0` = SDK がデバイスの TCP 7701 に `set_stream_buffer` を送り、連続再生モード（hold-decay + drift 補正、再prime なし）に切り替える
- 送信タイミング: ストリーム開始時の既知デバイス、および接続後に新しく見つかったデバイスへ、値ごとに 1 回だけ
- デバイスの TCP 7701 は単一クライアントスロットのため、Studio / Helper が接続中だと送信は失敗しますが、UDP 音声送出には影響しません（次回セッションで再試行）
