...
同一プレイヤーが複数の Hapbeat を装着し、それぞれ別々の haptics を出す場合は、**position の利用を推奨**する。player や group でも実現はできるが、部位の識別は position が意味的に自然であり、後からプレイヤーを増やす際に player 軸が空いている利点もある。
...
:::note[実行時コストはありません]
上書きが走るのは **Play / Stop / StopAll / StreamBegin を送る瞬間だけ**で、フレームごとやストリームのチャンクごとではありません。ストリーミング中の `STREAM_DATA` は対象外です。

さらに、override が両軸とも無効なとき（既定）は**入力をそのまま返して即座に抜ける**ため、アドレス文字列の処理自体が発生しません。有効な場合も、5 セグメント程度の文字列を 1 回組み立てるだけです。
:::
