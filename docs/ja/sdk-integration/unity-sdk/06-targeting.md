...
| 送信：1 / 受信：1 人が複数装着 | **position** 推奨 | 装着部位を設定し、ターゲットで撃ち分け |
| 送信：1 / 受信：複数プレイヤー | **player** | 各デバイスに player 1, 2, 3 … を割り当て |
| 送信：N / 受信：N（1:1 のペアが N 組） | **group** + override | ペアごとに group を割り当て、アプリ側で固定 |
| 送信：複数アプリ / 受信：混在 | **group** | アプリごとに group の範囲を分割 |
...
同一プレイヤーの首と右腕を撃ち分ける構成では、**position の利用を推奨**する。player や group でも実現はできるが、部位の識別は position が意味的に自然であり、後からプレイヤーを増やす際に player 軸が空いている利点もある。
...
1 つのアプリが 3 人に別々の触覚フィードバックを出す構成では **player** で分ける。
...
- デバイス側に group 1〜5 を割り当て
- 各 HMD のアプリで同じ番号を override として固定（→ [Override targeting](#override-targeting)）
- EventMap は 5 組で共通のまま使い回せる

**全端末に同一ビルドを配ったまま運用できる**のがこの方式の利点。ビルドを組ごとに分ける必要はない。

### 送信：複数アプリ / 受信：混在

デバイスは送信元アプリを区別しない。同じネットワークに複数のアプリが並ぶ場合は、**アプリごとに使う group の範囲を分割**する（例: App A = group 1〜10、App B = group 11〜20）。プレイヤー単位の制御が別途必要なら player 軸をそちらに残せるため、アプリの分離には group を充てるのが扱いやすい。

:::caution[OLED のアプリ名表示は group では分離できません]
触覚フィードバックは分離されますが、デバイスの OLED に出るアプリ名は最後に届いたもので上書きされます。アプリ名を運ぶ `CONNECT_STATUS` はファームウェア側で group による絞り込みを行わないため、group を分けても表示は分離されません（ルーティングをターゲット文字列に一本化した際の仕様変更によるものです）。

表示で「自分のアプリと繋がっているか」を確認する用途には使えません。
:::
