...
全文を渡すと文脈が無駄に肥大化します。まず **索引** を渡し、必要なツール/SDK の詳細だけをエージェントに取りに行かせるのが効率的です（Web 取得できる Claude / Cursor / browsing 有効な Copilot 等が対象）。

```text wrap
Hapbeat Developer Tools を使います。まず索引 https://devtools.hapbeat.com/llms.txt を読み、使うツール/SDK の詳細 txt（と、あれば curated な AGENTS.md）を必要に応じて取得して、その仕様に従ってください。
```

使う SDK/ツールが既に決まっているなら、各 Getting Started 冒頭の専用 URL を直接渡す方が速いです。
:::
