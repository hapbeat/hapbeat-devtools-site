...
Kit 名は **フォルダ名 + manifest.name + Event ID の prefix の 3 か所** で同じ文字列が使われます ([DEC-028](https://github.com/Hapbeat/hapbeat-sdk-workspace/blob/master/docs/decision-log.md))。命名規則は `^[a-z][a-z0-9-]*$` (英小文字始まり / 英数字とハイフン)。違反文字は入力時点で除去されます。
...
| 設定 | 説明 |
|---|---|
| **intensity** (Amp スライダー) | 基準振動強度 (0.0〜1.0)。Kit 設計時の "標準の強さ" として記録される ([gain の乗算構造](/docs/concepts/gain-architecture/) の基準値) |
| **Mode** | `> FIRE` / `♪ CLIP` / `>♪ BOTH` の 3-button radio。詳細: [Mode を切り替える](./modes/) |
...
- [Mode を切り替える](./modes/) — FIRE / CLIP / BOTH の Studio 上の操作
