...
An Event ID is a string that uniquely identifies a haptic event. The canonical format is defined by the [event-id spec](https://github.com/Hapbeat/hapbeat-contracts/blob/master/specs/event-id.md) (DEC-040).
...
| Uppercase | not allowed (normalized to lowercase) |
| `<file-name>` start | must be a letter (`sine_100hz`, not `100hz`) |
| Max total length | 255 characters |
