...
7. **Put the Hapbeat device on the same Wi-Fi, then press Play**
   Put on the HMD and grab a cube, press a button or drag a slider — haptics should follow.

:::tip[Deciding which event to wire]
Instead of step 6, run `Hapbeat > Samples > Augment XRI Hand Demo (+ diagnostic Event Logger)` to also log every XRI interactable event on the poke button to the Console. Useful while choosing wiring points, noisy otherwise.
:::
