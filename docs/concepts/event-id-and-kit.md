...
`install-clips/` と `stream-clips/` の使い分けは [Fire と Clip — どちらで送るか](./fire-vs-clip/) を参照。「install されてデバイスに残る」「stream として都度送る」という動詞対比で覚えると直感的です。
...
- [Fire と Clip — どちらで送るか](./fire-vs-clip/) — mode の選び方
- [gain の乗算構造](./gain-architecture/) — `intensity` が乗算チェーンのどこに入るか
- [Kit デザインガイド](/docs/tools/studio/kit-design/) — Studio で Kit を作る手順 (howto)
- [Mode の使い分け (Studio 視点)](/docs/tools/studio/modes/) — Studio UI 上での mode 切替
