...
`AnimatorController` の **state に直接 attach** する `StateMachineBehaviour` 派生。state Enter / Exit の瞬間に Animator runtime から直接呼ばれるため、scene 側 MonoBehaviour で Animator パラメータを poll する方式より遅延も発火条件も明快です。
