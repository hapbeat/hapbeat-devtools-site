...
EventMap の詳細: [EventMap ウィンドウ](/docs/sdk-integration/unity-sdk/event-map/)

## 5. Studio で Kit をデプロイして FIRE を有効化

Command モード（F キー）を動かすには、デバイスに `basic-exam-kit` をインストールします。Studio からのデプロイには **hapbeat-helper** が必要です（[初期セットアップ](/docs/tools/studio/initial-setup/) 参照）。
