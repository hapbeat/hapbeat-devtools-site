...
- Animator state Enter / Exit (AnimatorController の state 単位)
...
- HapbeatStateBehaviour (Animator state に直接 attach する StateMachineBehaviour)
- HapbeatUnityEventTrigger
- HapbeatSequenceTrigger (grab/hold/release を 1 体化)
- HapbeatTickEmitter (連続値スナップ)

参考: `Packages/com.hapbeat.sdk/Runtime/Hapbeat*Trigger.cs` および `HapbeatStateBehaviour.cs` を読んでから判断してください。
````
...
| 3 | ドア開閉 | Door | Animator state Open / Closed Enter | HapbeatStateBehaviour | low |
| ... |
```
...
- HapbeatStateBehaviour なら: AnimatorController asset + state 名 + OnStateEnter/Exit (Trigger は GameObject ではなく state に attach)
